...
class Drone1:
    ...
    def trajectory(self):
                ...
                while not rospy.is_shutdown():
                    ...
                    if state == actionlib.GoalStatus.ACTIVE:
                        ...
                        if raining_flag==True:
                            ...
                            self.machine._get_trigger(model=self,trigger_name='land')
                        if uav_2_flag==True:
        ...
        if result.surface_success==True:
            x=True
            surf_pub.publish(x)
        return uav_2_flag,k,a,self.state
          

    def explore_drone1(self):
        print(self.state)
        uav_2_flag,k,a,cur_stat=self.trajectory()
